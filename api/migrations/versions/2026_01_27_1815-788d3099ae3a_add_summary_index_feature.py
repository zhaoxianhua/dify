...
def upgrade():
    ...
    if _is_pg(conn):
        ...
        CREATE TABLE document_segment_summaries (
            ...
            summary_index_node_id VARCHAR(255) NULL,
            summary_index_node_hash VARCHAR(255) NULL,
            tokens INT NULL,
        ...
        op.execute("CREATE INDEX document_segment_summaries_chunk_id_idx ON document_segment_summaries(chunk_id);")
        op.execute("CREATE INDEX document_segment_summaries_dataset_id_idx ON document_segment_summaries(dataset_id);")
        op.execute("CREATE INDEX document_segment_summaries_document_id_idx ON document_segment_summaries(document_id);")
        op.execute("CREATE INDEX document_segment_summaries_status_idx ON document_segment_summaries(status);")

        op.execute("ALTER TABLE datasets ADD COLUMN summary_index_setting JSON NULL;")
        op.execute("ALTER TABLE documents ADD COLUMN need_summary TINYINT(1) NOT NULL DEFAULT 0;")
