    ...
    if _is_pg(conn):
        ...
        with op.batch_alter_table('embeddings', schema=None) as batch_op:
            conn.execute(sa.text(
                "ALTER TABLE embeddings ADD COLUMN model_name VARCHAR(40) NOT NULL DEFAULT 'text-embedding-ada-002'"))
            conn.execute(sa.text("ALTER TABLE embeddings DROP INDEX embedding_hash_idx"))
            #conn.execute(sa.text("ALTER TABLE embeddings ADD CONSTRAINT embedding_hash_idx UNIQUE (model_name, hash)"))
